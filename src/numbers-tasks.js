...
function getAverage(value1, value2) {
  const big1 = BigInt(value1);
  const big2 = BigInt(value2);

  return (big1 + big2) / BigInt(2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalear = x1 * x2 + y1 * y2;
  const modA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const result = Math.acos(scalear / (modA * modB));
  return result;
}
...
function isPowerOfTwo(num) {
  ...
  if (Number.isInteger(pow) && 2 ** pow === num) {
    return true;
  }
  return false;
}
